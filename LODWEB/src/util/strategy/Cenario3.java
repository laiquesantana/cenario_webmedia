...
public class Cenario3 {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < listUsers.length; i++) {
			...
			// List<Ratings> testSetRelevant =
			// DBFunctions.createTestSetByMin(listUsers[i], limitMin, 4);
			// List<Ratings> testSetIrrelevant =
			// DBFunctions.createTestSetByMax(listUsers[i], limitMax, 3);

			// testSet.addAll(testSetRelevant);
			// testSet.addAll(testSetIrrelevant);

			// retorna a lista de id de filmes
			// List<Integer> testSetList =
			// TaggingFactory.createTestSetList(testSet);
